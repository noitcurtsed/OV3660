...
#if CONFIG_EXAMPLE_CAM_I2C_PROBE
static void log_camera_bus_probe(i2c_master_bus_handle_t bus)
{
    static const uint8_t addrs[] = {
        0x30, /* OV2640, many sensors */
        0x36, /* OV5647 / others */
        0x3c, /* OV3660 / OV5640 default */
        0x3d, /* OV3660 alternate SID */
        0x21, /* GC0308 family */
        0x48, /* MT9D111 */
    };
...
void example_sensor_init(example_sensor_config_t *sensor_config, example_sensor_handle_t *out_sensor_handle)
{
...
#if CONFIG_EXAMPLE_CAM_I2C_PROBE
    ESP_LOGI(TAG, "SCCB bus port=%d SDA=%d SCL=%d pullups=%d freq=%d Hz",
               sensor_config->i2c_port_num,
               sensor_config->i2c_sda_io_num,
               sensor_config->i2c_scl_io_num,
               CONFIG_EXAMPLE_CAM_I2C_PULLUP_INTERNAL,
               EXAMPLE_CAM_SCCB_FREQ);
#endif
    ...
    i2c_master_bus_config_t i2c_bus_conf = {
        ...
        .flags.enable_internal_pullup = CONFIG_EXAMPLE_CAM_I2C_PULLUP_INTERNAL,
    };
    i2c_master_bus_handle_t i2c_bus_handle = NULL;
    ESP_ERROR_CHECK(i2c_new_master_bus(&i2c_bus_conf, &i2c_bus_handle));

#if CONFIG_EXAMPLE_CAM_I2C_PROBE
    log_camera_bus_probe(i2c_bus_handle);
#endif
